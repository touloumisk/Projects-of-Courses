...
void bmod ( double *a, double *b, double *c, long dimi, long dimj, long dimk, long stride )
{
	long j, k;
	double alpha;
//#pragma omp parallel for default(shared) private(j) schedule(dynamic) collapse(1)
	for ( k=0; k<dimk; k++ ) {
		for ( j=0; j<dimj; j++ ) {
			alpha = -b[k+j*stride];
...
{
	long i;
	v4sf X,Y,acc,temp,temp1,alph;

	//v4sf(double f)
	//{
	//val = (_v4sf){f,f,f,f};
	//}
	alph = (v4sf){alpha,alpha,alpha,alpha};


	acc = __builtin_ia32_xorps(acc,acc);
	temp = __builtin_ia32_xorps(temp,temp);

	for ( i=0; i<(n-3); i+=4 ) {
	X = __builtin_ia32_loadups(&a[i]);
	Y = __builtin_ia32_loadups(&b[i]);
	temp = __builtin_ia32_mulps(Y,alph);
	temp1 = __builtin_ia32_addps(X,temp);
	acc = __builtin_ia32_addps(acc, temp1);
	__builtin_ia32_storeups(a,acc);
	//	a[i] += alpha*b[i];
	}
...
#pragma omp parallel for default(shared) schedule(dynamic) collapse(1) ordered
	for ( k=0; k<n; k+=bs) {
		kl = k+bs;
		if ( kl>n ) {
			kl = n;
